...
export async function createBillingRecordInStrapi(
  ...
  // NOTA: Si el payload incluye status, usarlo (para adelantos/abonados explícitos)
  let status: PaymentStatus;
  
  if (payload.status) {
    // Usar el status proporcionado en el payload
...
export async function updateBillingRecordInStrapi(
  ...
  if (payload.status) {
    ...
    if (currentResp.ok) {
      ...
      const currentStatus = currentData.data?.status as PaymentStatus;
  ...
  if (payload.parentRecord !== undefined) {
    if (payload.parentRecord === null) {
      ...
      const parentResponse = await fetch(
        `${STRAPI_BASE_URL}/api/billing-records/${payload.parentRecord}?fields[0]=id`,
        { headers: { Authorization: `Bearer ${STRAPI_API_TOKEN}` }, cache: "no-store" }
      );

      if (parentResponse.ok) {
        const parentData = await parentResponse.json();
        const parentNumericId = parentData.data?.id;
...
export async function getStrapiNumericId(documentId: string): Promise<number | null> {
  try {
    const resp = await fetch(
      `${STRAPI_BASE_URL}/api/billing-records/${documentId}?fields[0]=id`,
      { headers: { Authorization: `Bearer ${STRAPI_API_TOKEN}` }, cache: "no-store" }
    );
    if (resp.ok) {
...
export async function findClosestParentRecord(
  ...
  try {
    ...
    // Fecha del nuevo pago
    const newPaymentDate = new Date(paymentDate);

    // Separar explícitamente por estado para prioridad clara
    const overdueCandidates = candidatesNeedingCoverage
      .filter(c => c.status === "retrasado")
      .sort((a, b) => {
    ...
    const pendingCandidates = candidatesNeedingCoverage
    ...
    console.log("[findClosestParentRecord] Overdue candidates:", overdueCandidates.map(c => ({
      documentId: c.documentId, dueDate: c.dueDate, balanceReal: c.balanceReal,
    })));
    console.log("[findClosestParentRecord] Pending candidates:", pendingCandidates.map(c => ({
      documentId: c.documentId, dueDate: c.dueDate, balanceReal: c.balanceReal,
    })));

    // Prioridad 1: Retrasado más antiguo
    if (overdueCandidates.length > 0) {
      const selected = overdueCandidates[0];
      console.log("[findClosestParentRecord] Selected overdue quota:", {
    ...
    // Prioridad 2: Pendiente más antiguo
    if (pendingCandidates.length > 0) {
      const selected = pendingCandidates[0];
      console.log("[findClosestParentRecord] Selected upcoming quota:", {
        documentId: selected.documentId,
        dueDate: selected.dueDate,
        balanceReal: selected.balanceReal,
      });
    ...
    // Fallback: cualquier candidato disponible (no debería llegar aquí)
    const selected = candidatesNeedingCoverage[0];
    console.log("[findClosestParentRecord] Selected (fallback):", {
      documentId: selected.documentId,
      dueDate: selected.dueDate,
      balanceReal: selected.balanceReal,
    });
...
export async function checkAndUpdateParentIfPaid(parentDocumentId: string): Promise<boolean> {
  try {
    ...
    const response = await fetch(`${STRAPI_BASE_URL}/api/billing-records/${parentDocumentId}?${query}`, {
      headers: {
        Authorization: `Bearer ${STRAPI_API_TOKEN}`,
        "Content-Type": "application/json",
      },
    ...
    // Solo procesar si el status es pendiente o retrasado
 ...
 */
async function applyAdvanceAsPartialPayment(
  advance: { documentId: string; numericId: number; available: number },
  ...
  try {
    ...
    // 3. Marcar el adelanto como pagado (todo su saldo fue consumido)
    const advanceUpdateResponse = await fetch(
      `${STRAPI_BASE_URL}/api/billing-records/${advance.documentId}`,
      {
        ...
        body: JSON.stringify({
          data: {
            status: "pagado",
            paymentDate: new Date().toISOString().split("T")[0],
            comments: `Consumido completamente. Saldo aplicado como abono parcial a cuota #${quota.quotaNumber}`,
          },
        }),
        cache: "no-store",
      }
    ...
    if (!advanceUpdateResponse.ok) {
      console.error(`[applyAdvanceAsPartialPayment] Error actualizando adelanto:`, await advanceUpdateResponse.text());
      // El abono ya fue creado, pero el adelanto no se marcó como pagado
      // Continuamos de todos modos para no dejar inconsistencias peores
      return true;
    }

    console.log(`[applyAdvanceAsPartialPayment] ✓ Adelanto ${advance.documentId} marcado como pagado`);
    return true;
  } catch (error) {
 ...
 * - Busca TODAS las cuotas pendientes/retrasadas del financiamiento
 * - Vincula cada cuota a un adelanto con suficiente saldo (FIFO)
 * - Si un adelanto tiene saldo parcial, lo aplica como abono parcial
 * 
 * @param financingDocumentId - documentId del financiamiento
 * @returns array de cuotas cubiertas o abonadas
 */
export async function autoCoverPendingQuotas(
  ...
  try {
    ...
    // 2. Buscar todas las cuotas pendientes/retrasadas
    const quotaQuery = qs.stringify({
      ...
      sort: ["dueDate:asc"],
      fields: ["id", "documentId", "amount", "quotaNumber", "dueDate"],
      pagination: { pageSize: 100 },
    }, { encodeValuesOnly: true });
    ...
    const pendingQuotas = (quotaData.data || []).map((q: any) => ({
      ...
      numericId: q.id,
      amount: (q.attributes || q).amount || 0,
      quotaNumber: (q.attributes || q).quotaNumber,
      dueDate: (q.attributes || q).dueDate,
    }));

    console.log(`[autoCoverPendingQuotas] Cuotas pendientes: ${pendingQuotas.length}`, pendingQuotas.map((q: any) => `#${q.quotaNumber}=$${q.amount}`));

    if (pendingQuotas.length === 0) {
      console.log(`[autoCoverPendingQuotas] No hay cuotas pendientes`);
      return [];
    }
    ...
    // 3. Vincular cuotas a adelantos (FIFO por adelanto, por dueDate por cuota)
    const coveredQuotas: string[] = [];

    for (const quota of pendingQuotas) {
      // 3a. Buscar primer adelanto con suficiente saldo (cobertura completa)
      const fullAdvance = advances.find((a: any) => a.available >= quota.amount);
      if (fullAdvance) {
        console.log(`[autoCoverPendingQuotas] Vinculando cuota #${quota.quotaNumber} ($${quota.amount}) a adelanto ${fullAdvance.documentId} (saldo: $${fullAdvance.available})`);
        ...
        if (updateResponse.ok) {
          fullAdvance.available -= quota.amount;
          fullAdvance.consumed += quota.amount;
      ...
      // 3b. Si no hay cobertura completa, intentar cobertura parcial con el primer adelanto con saldo > 0
      const partialAdvance = advances.find((a: any) => a.available > 0);
      if (partialAdvance && financingNumericId > 0) {
        console.log(`[autoCoverPendingQuotas] No hay saldo completo para cuota #${quota.quotaNumber}. Intentando cobertura parcial con adelanto ${partialAdvance.documentId} ($${partialAdvance.available})`);
        const applied = await applyAdvanceAsPartialPayment(partialAdvance, quota, financingNumericId);
        if (applied) {
          coveredQuotas.push(quota.documentId);
          // Marcar el adelanto como consumido en memoria para evitar reutilizarlo
          partialAdvance.available = 0;
          // Verificar si el abono parcial (junto con otros abonos previos) cubre el total
          await checkAndUpdateParentIfPaid(quota.documentId);
          console.log(`[autoCoverPendingQuotas] ✓ Cuota #${quota.quotaNumber} recibió abono parcial. Verificado si queda pagada.`);
        }
        continue;
      }

      console.log(`[autoCoverPendingQuotas] No hay adelanto con saldo para cuota #${quota.quotaNumber} ($${quota.amount})`);
    }
