  ...
  try {
    ...
    for (const file of files) {
      if (!(file instanceof File)) {
        return NextResponse.json({ error: "Uno de los archivos no es válido." }, { status: 400 });
      }
    ...
    console.log("[Upload] Subiendo a Strapi:", {
      ...
      files: files.map((f) =>
        f instanceof File ? { name: f.name, type: f.type, size: f.size } : null
      ),
      ref,
      refId,
      field,
    ...
    try {
      ...
      if (fetchError.name === "AbortError") {
        ...
        return NextResponse.json(
          {
            error:
              "La subida del archivo tardó demasiado. Intenta con un archivo más pequeño o verifica la conexión con el servidor.",
          },
          { status: 504 }
        );
      }
    ...
    if (!uploadedFile.id) {
      return NextResponse.json({ error: "Strapi no devolvió el ID del archivo." }, { status: 500 });
    }
