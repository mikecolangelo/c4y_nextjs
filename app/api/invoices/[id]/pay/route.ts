...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const getResponse = await fetch(`${STRAPI_BASE_URL}/api/invoices/${id}?populate=*`, {
      headers: {
    ...
    const updateResponse = await fetch(`${STRAPI_BASE_URL}/api/invoices/${id}`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${STRAPI_API_TOKEN}`,
    ...
    if (invoice.financing?.id) {
      ...
      const financingResponse = await fetch(`${STRAPI_BASE_URL}/api/financings/${financingId}`, {
        headers: {
          Authorization: `Bearer ${STRAPI_API_TOKEN}`,
        },
        cache: "no-store",
      ...
      if (financingResponse.ok) {
        ...
        await fetch(`${STRAPI_BASE_URL}/api/financings/${financingId}`, {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${STRAPI_API_TOKEN}`,
            "Content-Type": "application/json",
