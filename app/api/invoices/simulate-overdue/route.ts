...
function calculatePendingBalance(invoiceData: any): number {
  ...
  console.log(`[calculatePendingBalance] Cuota ${invoiceData.receiptNumber || 'N/A'}: Total=$${totalAmount}, Abonos=$${totalAbonos}, Pendiente=$${pendingBalance}`);
...
export async function POST(request: Request) {
  try {
    ...
    const coveredQuotasMap = await getCoveredQuotasMap(Array.from(financingDocumentIds), strapiToken);
