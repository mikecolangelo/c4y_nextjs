...
export async function GET(request: Request) {
  try {
    ...
    if (financingId) {
      ...
      console.log(`[API Billing] Found ${records.length} records`, records.map(r => ({ 
...
export async function POST(request: Request) {
  try {
    ...
    console.log(`[API Billing POST] Monto: ${data.amount}, quotaAmount: ${financing.quotaAmount}, quotasCovered: ${quotasCovered}`);
    
    // Calcular información del pago
    const paymentInfo = processPayment(data.amount, financing.quotaAmount, 0);
    console.log(`[API Billing POST] Monto: $${data.amount}, quotaAmount: $${financing.quotaAmount}, quotasCovered: ${paymentInfo.quotasCovered}, advanceCredit: $${paymentInfo.advanceCredit}`);
    ...
    if (!data.parentRecord && !isMultiQuotaPayment) {
      ...
      console.log(`[API Billing POST] Buscando cuota pendiente para vincular pago de $${data.amount}`);
    ...
    if (data.status === "adelanto") {
      console.log(`[API Billing POST] Adelanto detectado, forzando parentRecord = null y modo multi-cuota`);
    ...
    if (data.parentRecord) {
      console.log(`[API Billing POST] Checking if parent ${data.parentRecord} should be marked as paid after auto-association`);
    ...
    if (quotasCovered >= 2 && !data.parentRecord && record.documentId) {
      console.log(`[API Billing POST] Pago multi-cuota (${quotasCovered} cuotas), marcando cuotas como cubiertas`);
      ...
      console.log(`[API Billing POST] Marcadas ${coveredQuotas.length} cuotas como cubiertas por adelanto ${record.documentId}`);
    }
    ...
    if (record.status === "pendiente" && !data.parentRecord && record.documentId) {
      ...
      if (availableAdvanceId) {
        console.log(`[API Billing POST] Vinculando cuota ${record.documentId} a adelanto ${availableAdvanceId}`);
    ...
    if (autoCovered.length > 0) {
      console.log(`[API Billing POST] Auto-cover cubrió ${autoCovered.length} cuota(s):`, autoCovered);
    }
