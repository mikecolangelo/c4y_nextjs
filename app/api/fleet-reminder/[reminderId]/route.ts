...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    if (!reminderId) {
      return NextResponse.json({ error: "reminderId es requerido" }, { status: 400 });
    }
    ...
    const updateResponse = await fetch(`${STRAPI_BASE_URL}/api/fleet-reminders/${reminderId}`, {
      method: "PUT",
      headers: {
    ...
    if (!updateResponse.ok) {
      ...
      if (updateResponse.status === 404) {
        throw new Error(
          "El recordatorio no fue encontrado o el tipo de contenido 'fleet-reminders' no existe en Strapi."
        );
      }
...
export async function DELETE(_: Request, context: RouteContext) {
  try {
    ...
    const deleteResponse = await fetch(`${STRAPI_BASE_URL}/api/fleet-reminders/${reminderId}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${STRAPI_API_TOKEN}`,
      },
