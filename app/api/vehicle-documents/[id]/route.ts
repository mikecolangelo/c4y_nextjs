...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error("[vehicle-documents] PUT error:", response.status, errorText);
    ...
    console.error("[vehicle-documents] PUT error:", error);
...
export async function DELETE(_: Request, context: RouteContext) {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error("[vehicle-documents] DELETE error:", response.status, errorText);
      throw new Error(`Error ${response.status}: ${errorText || response.statusText}`);
    }

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("[vehicle-documents] DELETE error:", error);
    const message = error instanceof Error ? error.message : "Error desconocido";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
