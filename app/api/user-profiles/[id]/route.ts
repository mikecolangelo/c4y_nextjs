  ...
  try {
    ...
    const query = qs.stringify({
      ...
      populate: {
        ...
        driverHistories: {
          fields: ["id", "documentId", "startDate", "endDate", "status", "notes", "mileageStart", "mileageEnd"],
          populate: {
            vehicle: {
              fields: ["id", "documentId", "name", "vin", "brand", "model", "year", "currentMileage"],
              populate: {
                image: {
                  fields: ["url", "alternativeText"],
                },
        ...
        registeredVehicles: {
          fields: ["id", "documentId", "name", "vin", "brand", "model", "year", "currentMileage", "createdAt"],
          populate: {
            image: {
              fields: ["url", "alternativeText"],
            },
    ...
    try {
      ...
      const accountRes = await fetch(
        `${localStrapiUrl}/api/user-profiles/${id}/account`,
        {
          headers: {
            Authorization: `Bearer ${jwt}`,
          },
      ...
      if (accountRes.ok) {
        ...
        console.error("[API /user-profiles/:id] Error obteniendo account:", accountRes.status, await accountRes.text().catch(() => ''));
      }
    } catch (accountError) {
