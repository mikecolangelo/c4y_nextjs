...
interface VehicleDocumentsCardProps {
  ...
  onCreateCategory: (data: { name: string; description?: string; isActive: boolean; order: number }) => Promise<void>;
  onUpdateCategory: (id: string | number, data: { name?: string; description?: string; isActive?: boolean; order?: number }) => Promise<void>;
...
export function VehicleDocumentsCard({
  ...
  const renderFilePreview = (file: File, index: number, onRemove: (index: number) => void, keyPrefix: string) => (
    ...
    >
      ...
      <CardContent className={`flex flex-col ${spacing.gap.base} px-6 pb-6`}>
        {documents.length === 0 && !showForm && !editingDocument && !isLoadingDocuments && (
          <div className={`flex flex-col items-center justify-center py-10 ${spacing.gap.base} text-center`}>
            ...
            {documents.map((doc) => (
              ...
              >
                ...
                <div className={`flex flex-col ${spacing.gap.small} pr-24`}>
                  ...
                  {doc.description && (
                    <p className={typography.body.base}>{doc.description}</p>
                  )}
                  {doc.expirationDate && (
                ...
                {doc.files && doc.files.length > 0 && (
                  <div className={`mt-4 flex flex-col ${spacing.gap.small}`}>
                    <p className={`${typography.body.small} font-medium text-muted-foreground`}>Archivos</p>
                ...
                {doc.photos && doc.photos.length > 0 && (
                  <div className={`mt-4 flex flex-col ${spacing.gap.small}`}>
                    <p className={`${typography.body.small} font-medium text-muted-foreground`}>Fotos</p>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {doc.photos.map((photo, idx) => (
                        <div key={`doc-${doc.id}-photo-${idx}`} className="relative aspect-video w-full overflow-hidden rounded-lg bg-muted">
                          {photo.url ? (
                            <Image
                              src={photo.url}
        ...
        {showForm && !editingDocument && (
          <div className={`flex flex-col ${spacing.gap.small} ${documents.length > 0 ? "pt-4 border-t border-border" : ""}`}>
            <div className={`flex flex-col ${spacing.gap.small}`}>
              ...
              {!isLoadingCategories && activeCategories.length === 0 && (
                <p className="text-xs text-destructive">
                  No hay categorías activas. Haz clic en &quot;Gestionar Categorías&quot; para crear una.
                </p>
              )}
            ...
            {legalFiles.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {legalFiles.map((file, index) => renderFilePreview(file, index, handleRemoveLegalFile, "legal"))}
              </div>
            )}
            ...
            <div className={`flex flex-col ${spacing.gap.small}`}>
              ...
              >
                ...
                {legalFiles.length > 0 ? `Agregar más archivos (${legalFiles.length} seleccionados)` : "Haz clic para seleccionar archivos"}
              </Label>
              <Input
            ...
            {photoFiles.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {photoFiles.map((file, index) => renderFilePreview(file, index, handleRemovePhotoFile, "photo"))}
              </div>
            )}
            ...
            <div className={`flex flex-col ${spacing.gap.small}`}>
              ...
              >
                <Upload className="mr-2 h-5 w-5" />
                {photoFiles.length > 0 ? `Agregar más fotos (${photoFiles.length} seleccionadas)` : "Haz clic para seleccionar fotos"}
              </Label>
              <Input
            ...
            <div className="flex gap-2">
              <Button onClick={onCancelForm} variant="outline" size="lg" className="flex-1" disabled={isSaving}>
                Cancelar
              </Button>
              <Button
        ...
        {editingDocument && (
          <div className={`flex flex-col ${spacing.gap.small} pt-4 border-t border-border`}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-muted-foreground">(ID: {editingDocument.documentId || editingDocument.id})</span>
            </div>
            ...
            <div className={`flex flex-col ${spacing.gap.small}`}>
              ...
              <Select
                ...
                onValueChange={(value) => {
                  const cat = categories.find((c) => c.id.toString() === value || c.documentId === value);
                  setSelectedCategory(cat || null);
                }}
              >
                ...
                <SelectContent align="end">
                  {isLoadingCategories ? (
                    ...
                    <SelectItem value="empty" disabled>Sin categorías definidas</SelectItem>
                  ) : (
                    categories
              ...
              <div className={`flex flex-col ${spacing.gap.small}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 opacity-75">
                  {editingDocument.photos.map((photo, idx) => (
                    <div key={`edit-existing-photo-${idx}`} className="relative aspect-video w-full overflow-hidden rounded-lg bg-muted">
                      {photo.url ? (
                        <Image
                          src={photo.url}
                          alt={photo.alternativeText || photo.name || `Foto ${idx + 1}`}
            ...
            {legalFiles.length > 0 && (
              <div className={`flex flex-col ${spacing.gap.small}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {legalFiles.map((file, index) => renderFilePreview(file, index, handleRemoveLegalFile, "edit-legal"))}
                </div>
              </div>
            )}
            ...
            <div className={`flex flex-col ${spacing.gap.small}`}>
              ...
              >
                ...
                {legalFiles.length > 0 ? `Cambiar archivos legales (${legalFiles.length} seleccionados)` : "Seleccionar nuevos archivos legales (opcional)"}
              </Label>
              <Input
            ...
            {photoFiles.length > 0 && (
              <div className={`flex flex-col ${spacing.gap.small}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {photoFiles.map((file, index) => renderFilePreview(file, index, handleRemovePhotoFile, "edit-photo"))}
                </div>
              </div>
            )}
            ...
            <div className={`flex flex-col ${spacing.gap.small}`}>
              ...
              >
                <Upload className="mr-2 h-4 w-4" />
                {photoFiles.length > 0 ? `Cambiar fotos (${photoFiles.length} seleccionadas)` : "Seleccionar nuevas fotos (opcional)"}
              </Label>
              <Input
            ...
            <div className="flex gap-2">
              <Button onClick={onCancelEdit} variant="outline" size="lg" className="flex-1" disabled={isUpdating}>
                Cancelar
              </Button>
              <Button
