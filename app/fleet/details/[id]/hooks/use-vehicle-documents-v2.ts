...
export function useVehicleDocumentsV2(vehicleId: string): UseVehicleDocumentsV2Return {
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/vehicle-documents?vehicleDocumentId=${encodeURIComponent(vehicleId)}`,
        { cache: "no-store" }
      );
  ...
  const fetchCategories = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/vehicle-document-categories", { cache: "no-store" });
  ...
  const deleteDocument = async (docId: string | number) => {
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/vehicle-documents/${docId}`, { method: "DELETE" });
      if (!res.ok) {
        const err = await res.json();
  ...
  const createCategory = async (data: {
    ...
    try {
      const res = await fetch("/api/vehicle-document-categories", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const updateCategory = async (id: string | number, data: Partial<VehicleDocumentCategory>) => {
    ...
    try {
      const res = await fetch(`/api/vehicle-document-categories/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ data }),
      });
  ...
  const deleteCategory = async (id: string | number) => {
    ...
    try {
      const res = await fetch(`/api/vehicle-document-categories/${id}`, { method: "DELETE" });
      if (!res.ok) {
        const err = await res.json();
