    ...
    <Card className="shadow-sm ring-1 ring-inset ring-border/50 relative">
      <div className="absolute top-2 right-2 flex gap-1 z-10">
        ...
        {!onToggleCompleted && isCompleted && (
          <div
            className="flex items-center justify-center h-7 w-7 rounded-full bg-green-500/10"
            title="Completado"
          >
            <CheckCircle2 className="h-4 w-4 text-green-600" />
          </div>
      ...
      <div
        className={`flex flex-col ${spacing.gap.small} p-4 ${onEdit || onDelete ? "pr-12" : ""}`}
      >
        <div className="flex items-start gap-3">
          ...
          >
            {reminder.reminderType === "recurring" ? (
              <Repeat
                className={`h-4 w-4 ${reminder.isActive ? "text-primary" : "text-muted-foreground"}`}
              />
            ) : (
              <Bell
                className={`h-4 w-4 ${reminder.isActive ? "text-primary" : "text-muted-foreground"}`}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <p className={`${typography.body.small} font-semibold`}>{reminder.title}</p>
        ...
        {reminder.description && (
          <p className={`${typography.body.base} text-muted-foreground`}>{reminder.description}</p>
        )}
        ...
        <div className={`flex flex-col ${spacing.gap.small} pt-2 border-t border-border`}>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className={typography.body.small}>{formattedScheduledDate}</p>
            </div>
          </div>
          ...
          {reminder.reminderType === "recurring" && (
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className={typography.body.small}>{formattedNextTrigger}</p>
              </div>
            </div>
          )}
          ...
          {reminder.reminderType === "recurring" && reminder.recurrenceEndDate && (
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className={`${typography.body.small} text-muted-foreground`}>Finaliza:</p>
                <p className={typography.body.small}>
                  {format(new Date(reminder.recurrenceEndDate), "d 'de' MMMM, yyyy", {
                    locale: es,
                  })}
                </p>
              </div>
                  ...
                  <div className="flex items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity">
                    {user.avatar?.url ? (
                      ...
                      <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary/10 ring-2 ring-background">
                        <span
                          className={`${typography.body.small} font-semibold text-primary text-xs`}
                        >
