...
export function ServiceOrders({ compact = false, tall = false }: ServiceOrdersTimelineProps) {
  ...
  const loadOrders = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch("/api/service-orders-v2?populate=*", { cache: "no-store" });
      if (!response.ok) {
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    for (const orderId of idsToDelete) {
      try {
        const res = await fetch(`/api/service-orders-v2/${orderId}`, {
          method: "DELETE",
          cache: "no-store",
        });
        if (res.ok) {
          ...
          errorCount++;
          console.error(`Error deleting order ${orderId}:`, res.status);
        }
      } catch (err) {
        errorCount++;
    ...
    if (successCount > 0) {
      toast.success(`${successCount} orden${successCount > 1 ? "es" : ""} eliminada${successCount > 1 ? "s" : ""} correctamente`);
    }
    if (errorCount > 0) {
                  ...
                  {groupedOrders().map(([dateKey, dayOrders]) => (
                    <div key={dateKey} className="mb-4">
                      ...
                      <div className="flex items-center gap-2 mb-2 sticky top-0 bg-card/95 backdrop-blur-sm z-10 py-1">
                        <Checkbox
                          className="h-4 w-4"
                          checked={dayOrders.length > 0 && dayOrders.every((o) => selectedOrderIds.has(o.documentId || o.id))}
                          onCheckedChange={() => toggleDaySelection(dayOrders.map((o) => o.documentId || o.id))}
                        ...
                        {dayOrders.map((order) => (
                          <Card
                            ...
                            className={cn(
                              ...
                              selectedOrderIds.has(order.documentId || order.id) && "bg-primary/5 ring-primary/30"
                            )}
                            onClick={() => {
                              const targetId = order.documentId || order.id;
                              if (targetId) {
                                router.push(`/service-orders-v2/${targetId}`);
                              } else {
                                  ...
                                  <div className="flex flex-col items-center gap-1">
                                    ...
                                    >
                                      <Checkbox
                                        ...
                                        onCheckedChange={() => toggleOrderSelection(order.documentId || order.id)}
                                  ...
                                  <div className="min-w-0 flex-1">
                                    ...
                                    {order.totalCost !== undefined && order.totalCost > 0 && (
                                      <div className="flex items-center gap-1.5 mt-1 text-muted-foreground">
                                        ...
                                        <span className="text-xs font-medium">
                                          {new Intl.NumberFormat("es-PA", { style: "currency", currency: "PAB" }).format(order.totalCost)}
                                        </span>
                                      </div>
                                    )}
