      ...
      try {
        ...
        if (remindersResponse.ok) {
          ...
          const activeReminders = (data || []).filter((r: any) => 
            normalizeBoolean(r.isActive) && !normalizeBoolean(r.isCompleted)
          );
  ...
  const handleMarkAsRead = async (notificationId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setManualNotifications(prev => prev.filter(n => 
          (n.documentId || String(n.id)) !== notificationId
        ));
      }
    } catch (error) {
                  ...
                  return (
                    <div key={`manual-${notificationId}`}>
                      <DropdownMenuItem asChild className="p-0">
                        <div className="flex items-start gap-2 w-full p-3 hover:bg-accent cursor-pointer"
